...
```
① トピックシード更新
   → 毎週日曜23:00にClaude APIが翌週21本分のトピックを自動生成
   → Notion Config の weekly_topics に自動書き込み

② 採点3回連続不合格
   → スキップして次の投稿サイクルに委ねる
   → 失敗理由（スコア・コメント）はNotion Posts に status=failed で記録
   → 人間は後でNotionを見れば何が起きたか分かる

③ リンク投稿（アフィリ）の比率
   → pipeline.py 内でカウンターを持ち「5投稿に1回」をコードで固定
   → 変更するときはコードを修正してPR → マージ

④ マルチアカウント対応
   → 設定を config/accounts/{account_name}.yaml で管理
   → pipeline.py は account_name を引数で受け取る設計
   → 将来的に同じコードで複数アカウントを動かせる
```
